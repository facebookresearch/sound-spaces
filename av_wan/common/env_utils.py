...
from av_wan.common.sync_vector_env import SyncVectorEnv
...
def construct_envs(
    config: Config, env_class: Type[Union[Env, RLEnv]], workers_ignore_signals=False
) -> VectorEnv:
...
def make_env_fn(
    ...
    if len(config.DATASET_FILTER) != 0:
        dataset.filter_by_ids(config.DATASET_FILTER)
